...
const Experience = ({lightTheme}) => {
  const techs = [
    ...
    {
      icon: <BsDatabaseAdd  size={80} aria-label='databases icon' />,
      name: "Datenbanken",
      progress: 60,
      points:[
    ...
    {
      icon: <FaNodeJs  size={80} aria-label='node icon' />,
      name: "Nodejs",
      progress: 30,
      points:[
        "Erstellung von Servern",
        "Verwendung von Express",
  ...
  const [hoveredTech, setHoveredTech] = useState(techs[0]);

  return (
    <div name="experience" id="techstack" className={`${lightTheme ? 'bg-white':'bg-gradient-to-b from-black to-gray-500'} w-full h-auto p-4`}>
      <div className='max-w-screen-lg mx-auto flex flex-col justify-center w-full h-auto text-white mt-[80px]'>

        <div>
          <p className={`text-4xl font-bold border-b-4 border-blue-800 p-2 inline font-signature ${lightTheme ? 'text-black': 'text-white'}`}>Tech Stack</p>
          <p className={`py-6  ${lightTheme ? 'text-black': 'text-white'}`}>Im Arbeitsalltag habe ich Erfahrungen mit folgenden Techs gesammelt:</p>
        </div>
            ...
            {techs.map((tech, index) => (
              <div
                ...
                className={`transition-all duration-500 rounded-lg py-2 md:hover:scale-105 md:hover:drop-shadow-lg ${
                  ...
                  tech.name === "React" ? "md:hover:drop-shadow-blue-500" :
                  tech.name === "Tailwind" ? "md:hover:drop-shadow-sky-500" :
                  tech.name === "Nodejs" ? "md:hover:drop-shadow-[0_4px_6px_rgba(34,197,94,0.5)]" :
                  "md:hover:drop-shadow-purple-600"
                }`}
              >
                <div className={`flex justify-center items-center ${lightTheme ? 'text-black' : 'text-white'} `}>
                  {tech.icon}
                </div>
                <p className={`mt-4 ${lightTheme ? 'text-black' : 'text-white'}`}>{tech.name}</p>
              </div>
            ))}
          </div>
